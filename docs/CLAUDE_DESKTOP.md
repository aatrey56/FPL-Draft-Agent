...
The Go MCP server serves 33 tools over Streamable HTTP at `/mcp`, including the
decision layer: `draft_board`, `player_card`, `waiver_plan`, `my_week`,
`trade_check`, `league_pulse`, `drop_radar`. Claude (on a Max plan) is the
client — there is no in-app LLM.
